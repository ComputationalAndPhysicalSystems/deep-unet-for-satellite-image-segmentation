...
if __name__ == '__main__':
    ...
    test_id = "01"
    img = normalize(tiff.imread('/mnt/lfs2/epst0545/u_net_training/test_2/{}.tif'.format(test_id)).transpose([1,0,2]))   # make channels last
    ...
    for i in range(7):
        if i == 0:  # reverse first dimension
            ...
            mymat = np.mean( np.array([ temp[:,::-1,:], mymat.transpose([2,0,1])]), axis=0 )
            ...
            mymat = np.mean( np.array([ temp.transpose(2,0,1), mymat ]), axis=0 )
        elif i == 3:
            ...
            mymat = np.mean( np.array([ np.rot90(temp, -1).transpose([0,1,2]), mymat ]), axis=0 )
        elif i == 4:
            ...
            mymat = np.mean( np.array([ np.rot90(temp,-2).transpose([1,0,2]), mymat ]), axis=0 )
        elif i == 5:
            ...
            mymat = np.mean( np.array([ np.rot90(temp, -3).transpose(0,1,2), mymat ]), axis=0 )
        else:
            ...
            mymat = np.mean( np.array([ temp, mymat.transpose([2,1,0]) ]), axis=0 )
